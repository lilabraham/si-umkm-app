...
// Interface ini mendefinisikan data yang diterima dari API /api/toko
interface Toko {
  id: string;
  name: string;
  imageUrl: string;
        ...
        {tokoToDisplay.length > 0 ? (
          ...
          >
            {tokoToDisplay.map((toko) => (
              <TokoCard
                key={toko.id}
                toko={toko} // Mengirim seluruh objek 'toko'
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    ...
    return { 
      ...
      revalidate: 10 // Re-generate halaman setiap 10 detik
    };
  } catch (error) {
