...
import { parseCookies } from 'nookies'; // ⬅️ gunakan parseCookies
      ...
      <div className="p-6 lg:p-10">
        ...
        <motion.div variants={itemVariants} className="rounded-xl shadow-md bg-white w-full border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {trainings.length > 0 ? (
                trainings.map((t: Training, index: number) => (
                  <motion.tr
                    key={t.id}
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end pt-2">
                  <motion.button type="submit" className="bg-blue-600 text-white font-semibold py-2 px-5 rounded-md" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    Simpan
                  </motion.button>
                </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    const sortedTrainings = trainings.sort(
      (a: Training, b: Training) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)
    );

    return { props: { initialTrainings: JSON.parse(JSON.stringify(sortedTrainings)) } };
  } catch {
