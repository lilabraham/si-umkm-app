// LOKASI FILE: src/pages/admin/pelatihan.tsx
...
interface AdminPelatihanPageProps {
  initialTrainings: Training[];
}

const AdminPelatihanPage: NextPage<AdminPelatihanPageProps> = ({ initialTrainings }) => {
  ...
  return (
    <AdminLayout>
      <div className="p-6 lg:p-10">
        ...
        <motion.div
          variants={itemVariants}
          className="rounded-xl shadow-md bg-white w-full border border-gray-200 overflow-x-auto"
        >
          ...
          >
            ...
            >
              <motion.button
                ...
                className="absolute top-3 right-3 text-gray-400 hover:text-gray-800 hover:bg-gray-100 rounded-full p-1.5 transition"
                whileTap={{ scale: 0.8 }}
              >
                <CloseIcon size={20} />
              </motion.button>
              <h2 className="text-xl font-bold mb-5 text-slate-800">
                {modalMode === 'add' ? 'Tambah Pelatihan Baru' : 'Edit Pelatihan'}
              </h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                <input
                  ...
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-2 focus:ring-blue-400"
                  required
                />
                <textarea
                  name="description"
                  ...
                  onChange={handleFormChange}
                  rows={4}
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-2 focus:ring-blue-400 resize-none"
                  required
                />
                <input
                  type="text"
                  name="schedule"
                  placeholder="Jadwal (Contoh: Sabtu, 20 Juli 2024)"
                  value={currentTraining.schedule || ''}
                  onChange={handleFormChange}
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-2 focus:ring-blue-400"
                  required
                />
                <input
                  type="text"
                  name="location"
                  placeholder="Lokasi (Contoh: Online via Zoom)"
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  try {
    ...
    const token = cookies.token ? await adminAuth.verifyIdToken(cookies.token) : null;
    ...
    if (res.ok) trainings = await res.json();
    trainings.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));

    return { props: { initialTrainings: JSON.parse(JSON.stringify(trainings)) } };
  } catch {
