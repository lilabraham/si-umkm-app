// src/pages/admin/persetujuan.tsx
import { useState } from 'react';
import type { GetServerSideProps, GetServerSidePropsContext, NextPage } from 'next';
import Head from 'next/head';
import { parseCookies } from 'nookies'; // ⬅️ pakai parseCookies
import { Check, X, UserCheck, Inbox, AlertTriangle, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import AdminLayout from '@/components/layout/AdminLayout';
import { auth } from '@/lib/firebase'; // untuk client-side action
...
const PersetujuanPage: NextPage<PersetujuanPageProps> = ({ initialPendingUsers }) => {
  ...
  const handleManageUser = async (targetUserId: string, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      if (!token) throw new Error('Tidak dapat memverifikasi sesi Anda. Silakan login ulang.');
      ...
      setPendingUsers((current) => current.filter((user) => user.uid !== targetUserId));
    } catch (err: any) {
  ...
  return (
    <AdminLayout>
      ...
      <div className="p-6 lg:p-10">
        <motion.header className="mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <motion.div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
            ...
            <AnimatePresence>
              {pendingUsers.length > 0 ? (
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Deskripsi Usaha</th>
                    ...
                    {pendingUsers.map((user) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <p className="text-sm text-gray-600 max-w-xs truncate" title={user.description}>
                            {user.description}
                          </p>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex gap-2 justify-end">
                            <motion.button
                              onClick={() => handleManageUser(user.uid, 'reject')}
                            ...
                            <motion.button
                              onClick={() => handleManageUser(user.uid, 'approve')}
                              disabled={loading === user.uid}
                ...
                <motion.div className="text-center py-20 px-6" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                  ...
                  <h3 className="mt-4 text-xl font-semibold text-gray-800">Kotak Masuk Kosong</h3>
                  <p className="mt-2 text-gray-500">Tidak ada pendaftaran penjual baru yang perlu ditinjau saat ini.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export const getServerSideProps: GetServerSideProps = async (context: GetServerSidePropsContext) => {
  try {
    const { getAuth, getFirestore } = await import('@/lib/firebaseAdmin'); // ⬅️ server-only
    ...
    const initialPendingUsers: PendingUser[] = pendingSnapshot.docs.map((doc: any) => ({
      uid: doc.id,
      displayName: (doc.data() as any).displayName || '',
      shopName: (doc.data() as any).shopName || '',
      whatsapp: (doc.data() as any).whatsapp || '',
      description: (doc.data() as any).description || '',
      email: (doc.data() as any).email || '',
    }));

    return { props: { initialPendingUsers: JSON.parse(JSON.stringify(initialPendingUsers)) } };
  } catch {
